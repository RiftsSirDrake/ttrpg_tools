...
(function(root){
	...
	function HexMap(el,attr){
		...
		this.style = {
			'default': { 'fill': '#cccccc','fill-opacity':1,'font-size':fs,'stroke-width':1,'stroke-opacity':1,'stroke':'#000000' },
		...
		this.setMapping = function(mapping){
 		...
 		this._origin = new Hexagon(0,0,mapping.layout);
			...
			range.r = {min:Infinity,max:-Infinity};
			for(let r in mapping.hexes){
				range.q.min = Math.min(range.q.min,mapping.hexes[r].q);
				range.q.max = Math.max(range.q.max,mapping.hexes[r].q);
				range.r.min = Math.min(range.r.min,mapping.hexes[r].r);
				range.r.max = Math.max(range.r.max,mapping.hexes[r].r);
			}
